...
If enabled, agents will report the warning-level events generated by Kubernetes (namespace independent). Note that Kubernetes Events should be used with `ED_LEADER_ELECTION_ENABLED` environment variable in order to observe events.
